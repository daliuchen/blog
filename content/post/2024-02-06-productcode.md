...
user -> gd: request screenshot
gd -> screenshotService: request screenshot
screenshotService -> mq: request screenshot

screenshotService --> gd: return task id
gd --> user: return task id
loop do
user -> gd: request screenshot status
gd --> user: return screenshot status
end


group async process
screenshotService -> mq: pull screenshot request
screenshotService -> screenshotService: screenshot
screenshotService -> oss: upload screenshot
screenshotService -> mq: push image key and result
end

gd ->> mq: pull screenshot result
gd -> gd: update screenshot status
